        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 进行路由跳转之前，把购买的产品信息通过请求的形式通知服务器，服务器进行存储 -->
                <a
                  href="javascript:"
                  @click="addShopcar"
                >加入购物车</a>
...
export default {
  ...
  computed: {
    ...
    // 如果服务器的数据没有请求回来，skuInfo这个对象是个空对象，【空对象.skuImageList】的值是undefined，因此我们至少将skuImageList的值设为一个空数组
    skuImageList() {
  ...
  methods: {
    ...
    // 表单元素修改商品个数
    changeSkuNum(event) {
      // event.target.value获取的是输入框内的内容，默认是string类型。
      // 通过*1，不仅能将得到的字符串转为number类型，而且能判断用户输入的是否为数值（因为非数值*1，得到的是NaN）
      let value = event.target.value * 1;
      // 如果用户输入的数据非法(输入的不是数字，或者输入负数)
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      } else {
        // 正常的输入（若输入的是小数，则将输入的小数取整）
    ...
    async addShopcar() {
      /*
        2.dispatch的作用是调用actions中对应的addOrUpdateShopcar函数
        而actions中的addOrUpdateShopcar函数使用了async+await，该函数的返回值是个promise对象
        因此，此处可以使用async和await来进行加入购物车成功/失败的回调
      */
      try {
        // 1.点击加入购物车按钮，通知服务器加入购物车的产品信息，并存储
        await this.$store.dispatch("detail/addOrUpdateShopcar", {
          skuId: this.$route.params.skuId,
          skuNum: this.skuNum,
        });
        /* 
          3.进行路由跳转，将产品信息带个下一级的路由
          简单数据：商品数量skuNum通过query参数传递
          复杂数据：商品的信息skuInfo，通过会话存储sessionStorage
        */

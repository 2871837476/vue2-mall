...
export default {
  ...
  methods: {
    ...
    // 修改某一商品的个数，需发送请求，通知服务器进行修改(节流)
    handler: throttle(async function (type, disNum, cart) {
      // 根据触发事件的方式，设置相应的商品数量变化量
      switch (type) {
        case "add":
          disNum = 1;
          break;
        case "minus":
          // 原商品数量大于1，则点减号，商品变化量为-1；原商品数量小于等于1，则点减号，商品数量不变
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change":
          if (isNaN(disNum) || disNum < 1) {
            // 输入非法的数据，则变化量为0，数量保持原样
            disNum = 0;
          } else {
            // 输入正常的数据：输入的新数量-原有数量
    ...
    async updateChecked(cart, event) {
      try {
        // event.target.checked获取到的是布尔值，接口要求的是string类型
        let isChecked = event.target.checked ? "1" : "0";
        // 如果修改数据成功，再次获取服务器中的购物车数据
